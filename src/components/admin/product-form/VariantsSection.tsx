      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 pr-8">
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange(index, "variantName", e.target.value)}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              onChange(index, "costPrice", Number(e.target.value))
            }
            className={inputClass}
          />
        </div>
      ...
      <div className="border-t border-gray-100 pt-4 mt-4">
        ...
        {variant.images && variant.images.length > 0 ? (
          <div className="flex flex-wrap gap-2 mb-4">
            {variant.images.map((img, i) => (
              <div
                key={i}
                className="relative w-24 h-24 rounded-lg border border-gray-200 overflow-hidden group/img bg-gray-50"
              >
                <img
                  src={img}
                  alt={`Variant ${index} - ${i}`}
        ...
        {inputMode === "url" ? (
          ...
          <div className="relative group/upload">
            <label
              className={`flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${uploading ? "bg-gray-50 border-gray-300" : "border-gray-300 hover:bg-gray-50 hover:border-button2"}`}
            >
              {uploading ? (
                ...
                <div className="flex flex-col items-center px-4 py-2">
                  <svg
                    className="w-6 h-6 text-gray-400 mb-2 group-hover/upload:text-button2 transition-colors"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
