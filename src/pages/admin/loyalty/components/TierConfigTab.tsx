      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            <th className="px-6 py-3 text-left font-semibold text-gray-600">Tier</th>
            <th className="px-6 py-3 text-left font-semibold text-gray-600">Min. Spend (VND)</th>
        ...
        <tbody>
          {tiers.map((tier) =>
            editingTier === tier.tier ? (
              <tr key={tier.tier} className="border-b border-gray-100 bg-blue-50/40">
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${TIER_COLORS[tier.tier]}`}>
                ...
                <td className="px-6 py-4">
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, minSpent: e.target.value }))}
              ...
              <tr key={tier.tier} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${TIER_COLORS[tier.tier]}`}>
                    {TIER_LABELS[tier.tier] ?? tier.tier}
                  </span>
                ...
                <td className="px-6 py-4 text-gray-800">{tier.discountPercent}%</td>
                <td className="px-6 py-4">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                      tier.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                    }`}
                  >
