  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <StatCard label="Total Cost Value" value={fmt(data.summary.totalCostValue)} color="blue" />
        <StatCard label="Total Retail Value" value={fmt(data.summary.totalSaleValue)} color="green" />
        <StatCard label="Total Units in Stock" value={fmtNum(data.summary.totalItems)} color="yellow" sub="units" />
      </div>

      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">Inventory Value by Branch</h3>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data.byBranch}>
            ...
            <Legend />
            <Bar dataKey="totalCostValue" name="Cost Value" fill="#3b82f6" radius={[4, 4, 0, 0]} />
            <Bar dataKey="totalSaleValue" name="Retail Value" fill="#10b981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <tbody className="divide-y divide-gray-50">
            {data.byBranch.map((row) => (
              <tr key={String(row._id)} className="hover:bg-gray-50">
                <td className="py-3 px-4 font-medium text-gray-800">{row.branchName}</td>
                <td className="py-3 px-4 text-right text-gray-600">{fmtNum(row.totalItems)}</td>
                <td className="py-3 px-4 text-right text-blue-700">{fmt(row.totalCostValue)}</td>
                <td className="py-3 px-4 text-right text-green-700">{fmt(row.totalSaleValue)}</td>
