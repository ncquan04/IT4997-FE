...
class ProductAsync {
  ...
  fetchProduct = createAsyncThunk<
    ...
    { categoryId?: string; page?: number; includeHidden?: boolean },
    { rejectValue: any }
  >(
    "product/fetch-list",
    async (
      payload: { categoryId?: string; page?: number; includeHidden?: boolean },
      { rejectWithValue },
    ) => {
      try {
        const { categoryId, page, includeHidden } = payload;
        const response = await fetchProducts(categoryId, page, includeHidden);
        if (!response) {
  ...
  deleteProduct = createAsyncThunk<string, string, { rejectValue: any }>(
    "product/delete",
